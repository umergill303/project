  ...
  try {
    ...
    const productsWithVariants = await Promise.all(
      orderProducts.map(async product => {
        ...
        let processedThumbnail = '/Noimage.jpg'
        try {
          if (typeof product.thumbnail === 'string') {
            const parsed = JSON.parse(product.thumbnail)
            if (Array.isArray(parsed) && parsed.length > 0) {
              processedThumbnail = parsed[0]
            }
          }
        }
        catch (error) {
        ...
        if (!product.variantId) {
          return {
            ...product,
            thumbnail: processedThumbnail, // Use processed thumbnail
            variant: null,
        ...
        return {
          ...product,
          thumbnail: processedThumbnail, // Use processed thumbnail here too
